...
def get_args_parser():
    ...
    parser.add_argument(
        "--semantic_loss", default=1.0, type=float, help="""Semantic Weight"""
    )
    parser.add_argument(
        "--resume",
        default=False,
    ...
    parser.add_argument(
        "--crop_height", default=96, type=int, help="""Height of crop image"""
    )
    parser.add_argument(
        "--crop_width", default=96, type=int, help="""Width of crop image"""
    )
    parser.add_argument(
        "--out_dim",
        default=65536,
    ...
    parser.add_argument(
        "--filter_path", default="", type=str, help="Filter of image list."
    )
    parser.add_argument(
        "--keep_num", default=1281167, type=int, help="Number of training images."
    )
    parser.add_argument(
        "--output_dir", default=".", type=str, help="Path to save logs and checkpoints."
    )
    parser.add_argument(
        "--saveckp_freq", default=5, type=int, help="Save checkpoint every x epochs."
    )
...
class SingleFolderDataset(torch.utils.data.Dataset):
    """Dataset for loading images from a single folder without class subdirectories."""
...
def train_dino(args):
    ...
    if os.path.isdir(args.data_path):
        ...
        # 서브디렉토리가 없거나, 서브디렉토리가 있지만 이미지 파일이 직접 있는 경우
        if len(subdirs) == 0:
            # 루트 디렉토리에 이미지 파일이 있는지 확인
            files = [
                f
    ...
    if args.arch in vits.__dict__.keys():
        ...
        teacher = swin.swin_base_patch4_window7_224(img_size=(args.height, args.width))
        ...
        teacher = swin.swin_small_patch4_window7_224(img_size=(args.height, args.width))
        embed_dim = student.num_features[-1]
        ...
        teacher = swin.swin_tiny_patch4_window7_224(img_size=(args.height, args.width))
        embed_dim = student.num_features[-1]
    ...
    part_classifier = DINOHead(
        embed_dim, args.partnum + 1, nlayers=args.parthead_nlayers
    ).cuda()
    part_classifier = nn.parallel.DistributedDataParallel(
        part_classifier, device_ids=[args.gpu], find_unused_parameters=True
    )
    part_loss = nn.CrossEntropyLoss(reduction="none")

    # multi-crop wrapper handles forward with inputs of different resolutions
    student = utils.MultiCropCondWrapper(
        student,
        DINOHead(
            embed_dim,
            args.out_dim,
...
def train_one_epoch(
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 200, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            weight_unit = build_onehot_semantic_weight(
                args.batch_size_per_gpu, setVal=1
            )
            semantic_weight = [weight_unit, weight_unit]
            _, teacher_feats = teacher(
                images[:2], semantic_weight
            ...
            semantic_weight = [
                torch.cat(semantic_weight)[torch.from_numpy(mask_idxs)]
            ] + [weight_unit for _ in range(2 + args.local_crops_number)]
            student_output, student_feats = student(
                [nimages.float()] + images, semantic_weight
            )
            feats = student_feats[
                : (nimages.shape[0] + args.batch_size_per_gpu * 2)
            ].permute(0, 2, 3, 1)
            if len(mask_idxs) != 0:
            ...
            loss1 = 1.0 * dino_loss(
                student_output[nimages.shape[0] :], teacher_output, epoch
            )

            # sum all losses
            if len(mask_idxs) != 0:
                semantic_weight1 = semantic_weight[0]
                semantic_weight2 = torch.cat(semantic_weight[1:3])[
