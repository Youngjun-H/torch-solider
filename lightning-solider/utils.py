...
def cosine_scheduler(
    ...
    Args:
        ...
        niter_per_ep: Number of iterations per epoch (must be finite positive integer)
        warmup_epochs: Number of warmup epochs
        start_warmup_value: Starting value for warmup

    Returns:
        numpy array of learning rate schedule
    """
    # 무한대 값 체크
    if isinstance(niter_per_ep, float) and (
        math.isinf(niter_per_ep) or math.isnan(niter_per_ep)
    ):
        raise ValueError(
            f"niter_per_ep must be a finite positive number, got: {niter_per_ep}. "
            "This usually means the dataloader length could not be determined."
        )

    # niter_per_ep를 정수로 변환 (Lightning에서 float로 전달될 수 있음)
    niter_per_ep = int(niter_per_ep)

    if niter_per_ep <= 0:
        raise ValueError(f"niter_per_ep must be positive, got: {niter_per_ep}")
    ...
    assert (
        len(schedule) == total_iters
    ), f"Schedule length {len(schedule)} != expected {total_iters}"
    return schedule
...
class MultiCropCondWrapper(nn.Module):
    ...
    def forward(self, x, semantic_weight):
        ...
        start_idx = 0
        for end_idx in idx_crops:
            _out, _feats = self.backbone(
                torch.cat(x[start_idx:end_idx]),
                torch.cat(semantic_weight[start_idx:end_idx]),
            )
            if start_idx == 0:
                output = _out
                outfeats = _feats
